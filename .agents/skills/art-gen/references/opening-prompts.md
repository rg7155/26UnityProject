...
>
> ⚠️ **1차본이 4칸으로 나왔고, 그대로 채택했다.**
> 이미지 모델은 개수를 세지 못한다. `ONLY FIVE`라고 대문자로 써도 4칸이 나왔다
> (가로 스캔 실측: 폭 83/78/78/83 덩어리 4개).
>
> **알면서 넘긴 이탈이다.** 화면 540px 에서 칸 하나는 45px 남짓이고 아무도 세지 않는다.
> "5초"는 대사(`ARC「버퍼는 5초」`)가 전달하며, 그림은 "링이 조금만 켜져 있다"만 전하면 된다.
> 픽셀 편집으로 5번째 칸을 붙여봤으나 링 곡률을 못 따라가 artifact 가 남아 포기했다.
> 재생성 비용 대비 이득이 없다고 판단했다.
>
> 프롬프트는 고쳐뒀다 — **개수 대신 시계 방향 위치로 지정**한다.
> 다시 뽑을 일이 있으면 이 버전을 쓴다. 앞으로 개수가 의미를 갖는 요소는
> 숫자가 아니라 위치로 지정할 것.
...
6. (Opening03 한정) 켜진 눈금 개수 — **채택본은 4칸이다.** 알면서 넘긴 이탈이라 반려 사유가 아니다
